<template>
  <section aria-labelledby="prompt-pool-title" class="border-b border-gray-200 py-6 dark:border-dark-700/60">
      ...
      <div class="divide-y divide-gray-100 dark:divide-dark-800">
        ...
        >
          ...
          <div class="flex flex-wrap items-center justify-end gap-1 border-t border-gray-100 pt-3 dark:border-dark-800 xl:flex-nowrap xl:border-0 xl:pt-0">
            ...
            <button type="button" class="btn btn-ghost btn-sm text-red-600 hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-950/30" @click="requestEndpointRemoval(endpoint)">{{ t('common.delete') }}</button>
          </div>
        </article>
      </div>
    </div>
    ...
    <ConfirmDialog
      ...
      danger
      @confirm="confirmEndpointRemoval"
...
function requestEndpointRemoval(endpoint: PromptAuditEndpointDraft) {
  pendingRemoval.value = endpoint
}
function confirmEndpointRemoval() {
