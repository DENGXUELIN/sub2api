...
type kiroForceCacheCandidate struct {
	convertStringTo  string
...
func applyKiroForceCacheCreation(ctx context.Context, group *Group, body []byte, model string) []byte {
	NormalizeGroupRuntimeFields(group)
	if group == nil || !group.EffectiveKiroCacheForceCreationEnabled() || len(body) == 0 {
		return body
	}
	if len(bytes.TrimSpace(body)) < 256 {
		return body
	}
	...
	candidate, ok := findKiroForceCacheCandidate(ctx, payload)
	if !ok || candidate.cumulativeTokens < kiroMinimumCacheableTokens(model) {
		return body
	}
	if candidate.convertStringTo != "" {
		setKiroForceCacheConvertedString(payload, candidate)
	} else if !setKiroCacheControlOnTarget(candidate.target) {
		return body
	}
...
func findKiroForceCacheCandidate(ctx context.Context, payload map[string]any) (kiroForceCacheCandidate, bool) {
	cumulativeTokens := 0
	var candidate kiroForceCacheCandidate
	found := false

	if tools, ok := payload["tools"].([]any); ok {
		for _, tool := range tools {
			cumulativeTokens += kiroTokensPerTool
			if hasKiroCacheControl(tool) {
				return kiroForceCacheCandidate{}, false
			}
			if toolMap, ok := tool.(map[string]any); ok {
				candidate = kiroForceCacheCandidate{
					target:           toolMap,
	...
	case string:
		cumulativeTokens += countKiroSystemBlockTokens(system)
		if strings.TrimSpace(system) != "" {
			candidate = kiroForceCacheCandidate{
				convertStringTo:  "system",
				value:            system,
				cumulativeTokens: cumulativeTokens,
			}
			found = true
	...
	case []any:
		for _, systemBlock := range system {
			cumulativeTokens += countKiroSystemBlockTokens(systemBlock)
			if hasKiroCacheControl(systemBlock) {
				return kiroForceCacheCandidate{}, false
			}
			if systemMap, ok := systemBlock.(map[string]any); ok && isKiroCacheableTextBlock(systemMap) {
				candidate = kiroForceCacheCandidate{
					target:           systemMap,
					cumulativeTokens: cumulativeTokens,
				}
	...
	default:
		cumulativeTokens += countKiroSystemBlockTokens(system)
		if hasKiroCacheControl(system) {
			return kiroForceCacheCandidate{}, false
		}
		if systemMap, ok := system.(map[string]any); ok && isKiroCacheableTextBlock(systemMap) {
			candidate = kiroForceCacheCandidate{
				target:           systemMap,
				cumulativeTokens: cumulativeTokens,
			}
	...
	for _, rawMessage := range messages {
		...
		case string:
			cumulativeTokens += countKiroMessageContentTokens(ctx, typed)
			if strings.TrimSpace(typed) != "" {
				candidate = kiroForceCacheCandidate{
					convertStringTo:  "message_content",
					message:          message,
					value:            typed,
					cumulativeTokens: cumulativeTokens,
				}
		...
		case []any:
			for _, rawBlock := range typed {
				cumulativeTokens += countKiroMessageContentTokens(ctx, rawBlock)
				if hasKiroCacheControl(rawBlock) {
					return kiroForceCacheCandidate{}, false
				}
				if blockMap, ok := rawBlock.(map[string]any); ok && isKiroCacheableTextBlock(blockMap) {
					candidate = kiroForceCacheCandidate{
						target:           blockMap,
						cumulativeTokens: cumulativeTokens,
					}
		...
		default:
			cumulativeTokens += countKiroMessageContentTokens(ctx, content)
			if hasKiroCacheControl(content) {
				return kiroForceCacheCandidate{}, false
			}
			if contentMap, ok := content.(map[string]any); ok && isKiroCacheableTextBlock(contentMap) {
				candidate = kiroForceCacheCandidate{
					target:           contentMap,
					cumulativeTokens: cumulativeTokens,
				}
...
func hasKiroCacheControl(value any) bool {
	typed, ok := value.(map[string]any)
	if !ok {
		return false
	}
	_, ok = typed["cache_control"]
	return ok
}

func isKiroCacheableTextBlock(block map[string]any) bool {
	if block == nil {
		return false
	}
...
func setKiroForceCacheConvertedString(payload map[string]any, candidate kiroForceCacheCandidate) {
	block := map[string]any{
		"type":          "text",
		"text":          candidate.value,
		"cache_control": kiroDefaultCacheControlMap(),
	}
	switch candidate.convertStringTo {
	case "system":
		payload["system"] = []any{block}
	case "message_content":
		if candidate.message != nil {
			candidate.message["content"] = []any{block}
		}
	}
}

func setKiroCacheControlOnTarget(target map[string]any) bool {
	if target == nil || hasKiroCacheControl(target) {
		return false
	}
	target["cache_control"] = kiroDefaultCacheControlMap()
	return true
}

func kiroDefaultCacheControlMap() map[string]any {
	return map[string]any{"type": "ephemeral", "ttl": kiroCacheTTLString(kiroCacheDefaultTTL)}
}

func kiroCacheTTLString(ttl time.Duration) string {
	if ttl >= kiroCacheOneHourTTL {
		return "1h"
	}
	return "5m"
}
