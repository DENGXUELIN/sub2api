...
func compositeDefaultModelsListCandidateIDs() []string {
	seen := make(map[string]struct{})
	ids := make([]string, 0)
	for _, platform := range []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformKiro, PlatformGrok} {
...
func groupSupportsOAuthOnlyFilter(platform string) bool {
	return isOAuthOnlyRestrictedPlatform(platform) || platform == PlatformComposite
}
